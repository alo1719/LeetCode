...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        g = [[0]*(query_row+2) for _ in range(query_row+2)]
        g[0][0] = poured
        ...
        # space optimization
        g = [[0]*(query_row+2) for _ in range(2)]
        g[0][0] = poured
        for i in range(query_row+1):
            for j in range(i, -1, -1):  # important
                g[(i+1)%2][j] = 0  # important
                if g[i%2][j] > 1:
                    g[(i+1)%2][j] += (g[i%2][j]-1)/2
                    g[(i+1)%2][j+1] += (g[i%2][j]-1)/2
                    g[i%2][j] = 1
        return g[query_row%2][query_glass]
# @lc code=end
