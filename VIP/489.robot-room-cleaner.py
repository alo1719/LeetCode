...
#
# https://leetcode.cn/problems/robot-room-cleaner/description/
#
# algorithms
# Hard (74.70%)
# Likes:    192
...
class Solution:
    def cleanRoom(self, robot):
        # 1. random move
        # TC: O(number of tries)  SC: O(1)
        for i in range(1000000):
            robot.clean()
            turn_left_number = random.randint(0, 3)
            for i in range(turn_left_number):
                robot.turnLeft()
            robot.move()
        # 2. backtrack
        # TC: O(n-m)  SC: O(n-m)  n: number of cells, m: number of obstacles
