...
# mask can be replaced by sum, valid may not be necessary
@cache
def f(i, mask, limited, valid):
    # f[i][mask][limited][valid] = f[i+1][mask][False][False] if not valid
    #                            = sum(f[i+1][mask|(1<<digit)][limited and digit == upper_bound][True] for digit in range(1-int(valid), upper_bound+1))
